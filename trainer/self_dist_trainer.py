...
class DistTrainer(BaseTrainer):
    def __init__(self, teacher_model, student_model, config, writer):
    ...
    def iter(self, source_batch, target_batch):
        ...
        #pred_s, feature_s = self.student_model.forward(img_s.cuda())
        pred_s, feature_s = self.student_model.forward(img_s.cuda(), source=True)
        pred_t, feature_t = self.student_model.forward(img_t.cuda(), source=False)
        ...
        pred_t_teacher, _ = self.teacher_model.forward(img_t.cuda(), source=False)
        student = F.softmax(pred_t, dim=1) # change to prob
        teacher = F.softmax(pred_t_teacher, dim=1)
        loss_kd = F.kl_div(student, teacher, reduction='none')

        # loss = loss_seg + loss_e + loss_metric
        loss = loss_seg + loss_e + loss_kd.mean()

        self.losses.loss_source = loss_s
        self.losses.loss_target = loss_t
        self.losses.loss_entropy = loss_e
        self.losses.loss_kd = loss_kd.mean()
    ...
    def train(self):
        ...
        for r in range(self.round_start, self.config.round):
            self.student_model = self.student_model.train()
            #self.source_all = get_list(self.config.gta5.data_list)
            self.target_all = get_list(self.config.cityscapes.data_list)#[:50]

            # save pseudo-label after first round
            if r!=0:
                self.cb_thres = self.gene_thres(self.config.cb_prop + self.config.thres_inc * r) # cb_prop=0.1, thres_inc=0
                self.save_pred(r)
            ...
            self.optim = torch.optim.SGD(
                self.student_model.optim_parameters(self.config.learning_rate),
...
##!!
            ...
            for epoch in range(self.config.epochs):
                for i_iter, source_batch in tqdm(enumerate(self.source_loader)):
                    ...
                    if i_iter % self.config.val_freq ==0 and i_iter != 0:
                        miou = self.validate()
                        # print('miou : %.2f, r : %d, epoch : %d, iter: %d'%(miou*100, r, epoch, i_iter)) ##!!
    ...
    def gene_thres(self, prop, num_cls=19):
        ...
        probs = {}
        freq = {}
        # this load only test set - cityscapes ##!! may be need to change if use source and target data both
        ...
        for index, batch in tqdm(enumerate(loader)):
            ...
            with torch.no_grad():
                #x1, _ = self.student_model.forward(img.to(self.device))
                x1, _ = self.student_model.forward(img.to(self.device), source=False)
            ...
            for i in range(num_cls):
                ...
                if i not in probs:
                    ...
                    probs[i].extend(cls_probs[::5]) # adding every 5th prob from all images

        growth = {}
        thres = {}
        for k in probs.keys(): # iter over classes
            cls_prob = probs[k]
            cls_total = len(cls_prob)
            freq[k] = cls_total
            cls_prob = np.array(cls_prob)
            cls_prob = np.sort(cls_prob) # again sort, the sorted values
    ...
    def save_pred(self, round):
        ...
        with torch.no_grad():
            for index, batch in tqdm(enumerate(loader)):
                ...
                #output, _ = self.student_model.forward(image.to(self.device))
                output, _ = self.student_model.forward(image.to(self.device), source=False)
    ...
    def save_model(self, iter):
        ...
        torch.save(self.student_model.state_dict(), osp.join(self.config["snapshot"], tmp_name))
        wandb.save(osp.join(self.config["snapshot"], tmp_name)) # save model online ##!!

    def validate(self):
        self.student_model = self.student_model.eval()
        ...
        with torch.no_grad():
            for index, batch in tqdm(enumerate(testloader)):
                image, label, _, _, name = batch
                #output, _ =  self.student_model(image.cuda())
                output, _ =  self.student_model(image.cuda(), source=False)
                label = label.cuda()
